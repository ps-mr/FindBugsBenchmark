 ...
 *
 * This class was previously the main driver for FindBugs analyses, but has been replaced by
...
public abstract class FindBugs {
	...
	public static void configureTrainingDatabases(IFindBugsEngine findBugs) throws IOException {
		if (findBugs.emitTrainingOutput()) {
			...
			if (!new File(trainingOutputDir).isDirectory())
				throw new IOException("Training output directory " + trainingOutputDir
						+ " does not exist");
			AnalysisContext.currentAnalysisContext().setDatabaseOutputDir(trainingOutputDir);
			// XXX: hack
			System.setProperty("findbugs.checkreturn.savetraining", new File(trainingOutputDir,
					"checkReturn.db").getPath());
		}
		if (findBugs.useTrainingInput()) {
			String trainingInputDir = findBugs.getTrainingInputDir();

			if (!new File(trainingInputDir).isDirectory())
				throw new IOException("Training input directory " + trainingInputDir
						+ " does not exist");
			...
			// XXX: hack
			System.setProperty("findbugs.checkreturn.loadtraining", new File(trainingInputDir,
					"checkReturn.db").getPath());
		} else {
	...
	public static boolean isDetectorEnabled(IFindBugsEngine findBugs, DetectorFactory factory,
		...
		if (!isNonReportingDetector && !reportedBugPatterns.isEmpty()) {
			...
			if (maxRank > rankThreshold) {
				if (false) {
					System.out.println("Detector " + factory.getShortName() + " has max rank "
							+ maxRank + ", disabling");
	 ...
	 */
	public static void processCommandLine(TextUICommandLine commandLine, String[] argv,
			IFindBugsEngine findBugs) throws IOException, FilterException {
	...
	public static void runMain(IFindBugsEngine findBugs, TextUICommandLine commandLine)
		...
		FutureValue<Collection<UpdateChecker.PluginUpdate>> updateHolder = null;
		if (verbose)
		...
		System.err.println("Overall analysis time: " + (endTime - startTime) / 1000.0 / 1000.0
				/ 1000.0);
			...
			if (updateHolder.isDone()) {
				try {
					...
					if (!DetectorFactoryCollection.instance().getUpdateChecker()
							.updatesHaveBeenSeenBefore(updates))
	 ...
	 */
	public static BugReporter configureFilter(BugReporter bugReporter, String filterFileName,
			boolean include) throws IOException, FilterException {
	...
	public static BugReporter configureBaselineFilter(BugReporter bugReporter,
			String baselineFileName) throws IOException, DocumentException {
	...
	public final static long MINIMUM_TIMESTAMP = new GregorianCalendar(1996, 0, 23).getTime()
			.getTime();
